...
$$
\Delta\theta_t=\alpha(\textcolor{red}{G_t} - v_\theta(s))\nabla_\theta v_\theta(s)
$$
  - For TD, the target is the TD target $R_{t+1}+\gamma v_\theta(S_{t+1})$
$$
\Delta\theta_t=\alpha(\textcolor{red}{R_{t+1}+\gamma v_\theta(S_{t+1})} - v_\theta(S_t))\nabla_\theta v_\theta(S_t)
$$
...
$$
v_\pi \approx \textcolor{red}{G_t}
$$
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <font color=blue>**LSTD**</font> Least Squares Temporal-Difference uses TD target
$$
v_\pi \approx \textcolor{red}{R_{t+1} + \gamma v_\theta(S_{t+1})}
$$
...
- Online neural Q-learning may include:
  + A **network** $q_\theta:\medspace O_t \Longrightarrow (q[1],\ldots,q[m])(m\space \text{actions})$
  + An $\epsilon-\text{greedy}$ **exploration policy**: $q_t\medspace \Longrightarrow \medspace \pi_t \Longrightarrow \medspace A_t$
...
- DQN (Mnih et al. 2013, 2015) includes;
  + A **network** $q_\theta:\medspace O_t \mapsto (q[1],\ldots,q[m])(m\space \text{actions})$
  + An $\epsilon-\text{greedy}$ **exploration policy**: $q_t\medspace \mapsto \medspace \pi_t \Longrightarrow \medspace A_t$
  + A **replay buffer** to store and sample past transitions
  + A **target network** $q_{\theta^-}:\medspace Q_t \mapsto\space(q^-[1],\ldots,q^-[m])$
...
### n-Step Return
---
- Consider the following n-steps returns for $n=1,2,\infin:$
$$
\begin{alignedat}a
